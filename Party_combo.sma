...
public on_damage(id)
{
    if(g_enabled)
    {
        ...
        if(g_showrecieved && zp_get_user_zombie(id))
        {    
...
public task_combo(attacker)
{
    ...
    if(g_combo[attacker]>0)
    {
        ...
        ShowSyncHudMsg(attacker, g_hudmsg1, "Combos Normales^nHits: %d | Damage: %d^n%d | %d^nTotal de Combos: %d^nTu Ganas: %d AMMO PACKS", 
...
public task_party_combo(i)
{
    ...
    if(g_combop[i]>0 && !zp_get_user_zombie(i) && is_user_alive(i))
    {
        if(g_PartyData[i][In_Party])
        {
            set_hudmessage(0, 200, 200, -1.0, 0.6, 1, 0.1, 4.0, 0.01, 0.01, -1)
            ShowSyncHudMsg(i, g_hudmsg1, "Combo de la Party^nHits: %d | Damage: %d^n%d | %d^nTotal de Combos: %d^nTu Ganas: %d AMMO PACKS", 
...
public reset_combo(id)
{
    ...
    if (g_combo[id]>0)
    {
        ...
        ColorChat(id, TEAM_COLOR, "^x04[ZP]^x01 Combo Total:^x04 %d^x01 | Damage Total:^x04 %d^x01 | Hits Echos:^x04 %d^x01 | Ganancia:^x04 %d", g_combo[id], g_damage[id], g_Hits[id], ganancia)
        zp_set_user_ammo_packs(id, zp_get_user_ammo_packs(id) + ganancia)
    }
...
public reset_party_combo(i)
{
    ...
    if(g_PartyData[i][In_Party])
    {
        ...
        {
            set_hudmessage(120, 120, 120, -1.0, 0.25, 0, 0.1, 4.0, 0.01, 0.01, -1)
            ShowSyncHudMsg(i, g_hudmsg1, "^n^n^n^n Combo Party Terminado (%d)^nGanancia: %d AMMO PACKS", g_combop[i], gananciap)
            ColorChat(i, TEAM_COLOR, "^x04[ZP]^x01 Combo party Total:^x04 %d^x01 | Damage Total:^x04 %d^x01 | Hits Echos:^x04 %d^x01 | Ganancia:^x04 %d", g_combop[i], g_damagep[i], g_Hitsp[i], gananciap)
        }
        zp_set_user_ammo_packs(i, zp_get_user_ammo_packs(i) + gananciap)
    }
